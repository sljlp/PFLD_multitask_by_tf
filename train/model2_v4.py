...
from lk.lk import lk_forward, lk_loss
...
def pfld_inference(input, weight_decay, batch_norm_params, isTrain=True):
    coefficient = 1
        ...
        if isTrain:
            with slim.arg_scope([slim.convolution2d, slim.separable_conv2d], \
                ...
                conv3_2 = slim.separable_convolution2d(conv3_2, num_outputs=None, stride=1, depth_multiplier=1,
                                                       kernel_size=[3, 3], scope='conv3_2/dwise')
                print(conv3_2.name, conv3_2.get_shape())
                conv3_2 = slim.convolution2d(conv3_2, 64 * coefficient, [1, 1], stride=1, activation_fn=None,
                                             scope='conv3_2/linear')
                print(conv3_2.name, conv3_2.get_shape())
                ...
                # box = np.zeros([config.batch_size,4],dtype=np.float32)
                # box = tf.convert_to_tensor(box,dtype=tf.float32)
                # multi_scale = tf.concat([multi_scale], 1)
                # multi_scale = slim.fully_connected(multi_scale,
                #                                    num_outputs=2 * config.attr_count + config.point_size * 2,
                #                                    scope='attr_fc1')
                # attr = slim.fully_connected(multi_scale, num_outputs=config.anno_attr_count, activation_fn=None,
                #                             scope='fc2')
                landmarks = slim.fully_connected(multi_scale, num_outputs=config.point_size * 2, activation_fn=None,
                                                 scope='fc')

                race3 = slim.fully_connected(multi_scale, num_outputs=3, activation_fn=None, scope='fc2_race3')
                race4 = slim.fully_connected(multi_scale, num_outputs=4, activation_fn=None, scope='fc2_race4')

                angles_gt = slim.fully_connected(multi_scale, num_outputs=3, activation_fn=None, scope='fc2_angle_gt')
                angles_st = slim.fully_connected(multi_scale, num_outputs=3, activation_fn=None, scope='fc2_angle_st')

                expressions = slim.fully_connected(multi_scale, num_outputs=10, activation_fn=None,
                                                   scope='fc2_expressions')

                attr6 = slim.fully_connected(multi_scale, num_outputs=5, activation_fn=None, scope='fc2_attr6_gt')
                # attr6_st = slim.fully_connected
                age = slim.fully_connected(multi_scale, num_outputs=1, activation_fn=None, scope='fc2_age')

                gender = slim.fully_connected(multi_scale, num_outputs=1, activation_fn=None, scope='fc2_gender')

                young = slim.fully_connected(multi_scale, num_outputs=1, activation_fn=None, scope='fc2_young')

                mask = slim.fully_connected(multi_scale, num_outputs=1, activation_fn=None, scope='fc_mask')

                open_eye = slim.fully_connected(multi_scale, num_outputs=1, activation_fn=None, scope='fc2_open_eye')

                open_mouth_degrees = slim.fully_connected(multi_scale, num_outputs=2, activation_fn=None,
                                                          scope='fc2_opem_mouth_dgrees')

                sunglasses = slim.fully_connected(multi_scale, num_outputs=1, activation_fn=None,
                                                  scope='fc2_sunglasses')

                forty_attrs = slim.fully_connected(multi_scale, num_outputs=33, activation_fn=None,
                                                   scope='fc2_forty_attrs')

                return features, landmarks, \
                       race3, race4, angles_gt, angles_st, expressions, attr6, age, gender, young, mask, open_eye, open_mouth_degrees, sunglasses, forty_attrs
        else:
            with slim.arg_scope([slim.convolution2d, slim.separable_conv2d], \
                                activation_fn=tf.nn.relu6, \
                                weights_initializer=tf.truncated_normal_initializer(stddev=0.01),
                ...
                # if box is None:
                #     box = np.zeros([16, 4], dtype=np.float32)
                #     box = tf.convert_to_tensor(box, dtype=tf.float32)

                # multi_scale = tf.concat([multi_scale, box], 1)
                # multi_scale = slim.fully_connected(multi_scale, num_outputs=2 * config.attr_count+config.point_size*2, scope='attr_fc1')
                # multi_scale = slim.batch_norm(multi_scale)
                # attr = slim.fully_connected(multi_scale, num_outputs=config.attr_count, activation_fn=None,
                #                             scope='attr_fc2')
                landmarks = slim.fully_connected(multi_scale, num_outputs=config.point_size * 2, activation_fn=None,
                                                 scope='fc')
                attr = slim.fully_connected(multi_scale, config.attr_count, activation_fn=None, scope='fc2')

                # attr = landmarks[-config.attr_count:]
                return features, landmarks, attr


def create_model(input, phase_train, args):
    '''

    :param input: [N1,H,W,C]
    :param landmark: [N1,pts_num]
    :param video: [N2*Seq,H,W,C]
    :param phase_train: [true or false]
    :param args:
    :return:
    '''
    batch_norm_params = {
    ...
    # features, landmarks_pre, attr_pre = pfld_inference(input, box, args.weight_decay, batch_norm_params)

    features, landmarks_pre, race3, race4, angles_gt, angles_st, expressions, attr6, age, gender, young, mask, open_eye, open_mouth_degrees, sunglasses, forty_attrs \
        = pfld_inference(input, args.weight_decay, batch_norm_params)
    ...
    return landmarks_pre, race3, race4, angles_gt, angles_st, expressions, attr6, age, gender, young, mask, open_eye, open_mouth_degrees, sunglasses, forty_attrs, euler_angles_pre


def create_model_multigpu(input, phase_train, args, reuse=False):
    '''
