...
def pfld_inference(input, box, weight_decay, batch_norm_params, isTrain=True):
    coefficient = 1
    with tf.variable_scope('pfld_inference'):
        ...
        if isTrain:
            with slim.arg_scope([slim.convolution2d, slim.separable_conv2d], \
                ...
                #                                    scope='attr_fc1')
                fc2_1 = slim.fully_connected(multi_scale, num_outputs=config.point_size * 2, scope='fc2_1',activation_fn=None)
                fc2_1 = slim.batch_norm(fc2_1,scope='fc2_1_bn')
                fc2_1 = tf.nn.relu(fc2_1,name='fc2_1_relu')

                fc2_1_1 = slim.fully_connected(fc2_1,num_outputs=config.point_size, scope='fc2_1_1', activation_fn=None)
                fc2_1_1 = slim.batch_norm(fc2_1_1,scope='fc2_1_1_bn')
                fc2_1_1 = tf.nn.relu(fc2_1_1,name='fc2_1_1_relu')

                fc2_2 = slim.fully_connected(fc2_1_1, num_outputs=config.attr_count * 2, scope='fc2_2',activation_fn=None)
                fc2_2 = slim.batch_norm(fc2_2, scope='fc2_2_bn')
                fc2_2 = tf.nn.relu(fc2_2,name='fc2_2_relu')

                attr = slim.fully_connected(fc2_2, num_outputs=config.anno_attr_count, activation_fn=None,
                                            scope='fc2')
...
def create_model(input, landmark, video, box, phase_train, args):
    '''
    ...
    lkLoss = tf.constant(0.0, dtype=tf.float32)
    if video is not None:
        features_video, video_landmarks_pre, attr_video_pre = pfld_inference(video, args.weight_decay,
                                                                             batch_norm_params)

        video_lmk_shape = tf.TensorShape([lk_config['video_count'], lk_config['video_seq'], config.point_size, 2])
        video_lmk_shape_for_loss = tf.TensorShape(
            [lk_config['video_count'] * lk_config['video_seq'] * config.point_size, 2])
        video_shape = tf.TensorShape(
            [lk_config['video_count'], lk_config['video_seq'], config.imgsize, config.imgsize, 3])

        video = tf.reshape(video, video_shape)
        video_landmarks_pre = tf.reshape(video_landmarks_pre, video_lmk_shape)
        next_batch, fBack_batch, back_batch, boxes = lk_forward(video_landmarks_pre * config.imgsize, video)

        video_landmarks_pre = tf.reshape(video_landmarks_pre, video_lmk_shape_for_loss)
        next_batch = tf.reshape(next_batch, video_lmk_shape_for_loss)
        fBack_batch = tf.reshape(fBack_batch, video_lmk_shape_for_loss)
        back_batch = tf.reshape(back_batch, video_lmk_shape_for_loss)
        lkLoss, validLmkNum = lk_loss(prediction=video_landmarks_pre, nextPts=next_batch, fbPts=fBack_batch,
                                      backpts=back_batch)
    ...
    return landmarks_pre, landmarks_loss, euler_angles_pre, lkLoss, attr_pre
