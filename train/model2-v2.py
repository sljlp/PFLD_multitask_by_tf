        ...
        if isTrain:
            with slim.arg_scope([slim.convolution2d, slim.separable_conv2d], \
                ...
                attr = slim.fully_connected(multi_scale, num_outputs=config.anno_attr_count, activation_fn=None,
                                            scope='attr_fc2')
                landmarks = slim.fully_connected(multi_scale,num_outputs=config.point_size*2,activation_fn=None,scope='fc')
            ...
            with slim.arg_scope([slim.convolution2d, slim.separable_conv2d], \
                ...
                #                             scope='attr_fc2')
                landmarks = slim.fully_connected(multi_scale,num_outputs=config.point_size*2+config.anno_attr_count,activation_fn=None,scope='fc')
                attr = landmarks[-config.attr_count:]
                return features, landmarks[:-config.attr_count], attr
...
def create_model(input, landmark, video, box, phase_train, args):
    ...
    if video is not None:
        ...
        features_video, video_landmarks_pre, attr_video_pre = pfld_inference(video,args.weight_decay, batch_norm_params)

        video_lmk_shape = tf.TensorShape([lk_config['video_count'],lk_config['video_seq'],config.point_size, 2])
        video_lmk_shape_for_loss = tf.TensorShape([lk_config['video_count'] * lk_config['video_seq'] * config.point_size,2])
        video_shape = tf.TensorShape([lk_config['video_count'], lk_config['video_seq'], config.imgsize,config.imgsize, 3])
        ...
        lkLoss, validLmkNum = lk_loss(prediction=video_landmarks_pre, nextPts=next_batch, fbPts= fBack_batch, backpts= back_batch)
